...
public final class fcsFile {
    ...
    public double[][] getCompensation() {
        ...
        // use the compensation matrix as is. In case $SPILL or $spillover
        // property had been used, we need to return the inverse of the 
        // compmatrix

        if (!("$COMP".equals(propertyUsed))) {
            RealMatrix matrixInverse = new LUDecompositionImpl(new Array2DRowRealMatrix(matrix)).getSolver().getInverse();
            matrix = matrixInverse.getData();
        }

        int startOfCompensatedElements = 0;
        String nameOfFirstEltToBeCompensated = null;
        Pattern p = Pattern.compile("<(.*)>");
        for (int i=0; i<9; i++) {
            String temp = compNames[i];
            Matcher m = p.matcher(temp);
                if (m.matches()) {
                    startOfCompensatedElements = i;              
                    nameOfFirstEltToBeCompensated = m.group(1);
                    break;
                }
            
        }
        
        int EltToBeCompensated = 0;
        for (int i=0; i<9; i++) {
            if (nameOfFirstEltToBeCompensated.matches(compNames[i])) {
                EltToBeCompensated = i;
                break;
            }
        }
        
        int numberOfAttributesToBeCompensated = startOfCompensatedElements - EltToBeCompensated;
        double[][] matrixofAttributesToBeCompensated = new double[n][numberOfAttributesToBeCompensated];
        for (int i=0; i<n; i++) {
            for (int j=EltToBeCompensated; j<EltToBeCompensated+numberOfAttributesToBeCompensated; j++) {
                matrixofAttributesToBeCompensated[i][j-EltToBeCompensated] = matrix[i][j];
            }
        }
        return matrixofAttributesToBeCompensated;
    }
    ...
    public double[][] calculateCompensation(double[][] compData,
        ...
        // compMatrix and rawMatrix of type Array2DRowRealMatrix respectively 
        Array2DRowRealMatrix compMatrix = new Array2DRowRealMatrix(compData);
        Array2DRowRealMatrix rawMatrix = new Array2DRowRealMatrix(rawData);

        // Calculate transpose of raw matrix
        RealMatrix rawMatrixTranspose = rawMatrix.transpose();

        // Apply formula: "transpose(compMatrix * transpose(events))"
        RealMatrix compMultRawDataTranspose = compMatrix.multiply(rawMatrixTranspose);
        RealMatrix finalResult = compMultRawDataTranspose.transpose();
